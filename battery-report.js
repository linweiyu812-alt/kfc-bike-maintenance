...
function syncCenter(resetRestaurant=true){
...
$("restaurantSearch").oninput=()=>{
 restaurant=null;
 $("restaurantSelected").textContent="";
 clearTimeout(timer);
 const q=$("restaurantSearch").value.trim();
 if(!q){$("restaurantResults").innerHTML="";return}
 $("restaurantResults").innerHTML='<div class="result">搜尋中…</div>';
 timer=setTimeout(searchRestaurant,120);
};
async function searchRestaurant(){
 const q=$("restaurantSearch").value.trim(),c=$("center").value;
 if(!q||!c){$("restaurantResults").innerHTML="";return}
 const {data,error}=await sb.rpc("public_restaurant_search",{p_center_code:c,p_keyword:q});
 if(error){$("restaurantResults").innerHTML=`<div class="result">搜尋失敗：${error.message}</div>`;return}
 const rows=(data||[]).slice(0,10);
 $("restaurantResults").innerHTML=rows.map((r,i)=>`<div class="result" data-r="${i}"><b>${r.name}</b>${r.store_no?`<small>店號：${r.store_no}</small>`:""}</div>`).join("")||'<div class="result">找不到符合的餐廳</div>';
 document.querySelectorAll("[data-r]").forEach(e=>e.onclick=()=>{
   restaurant=rows[+e.dataset.r];
...
document.addEventListener("DOMContentLoaded",()=>syncCenter(false));
window.addEventListener("pageshow",()=>syncCenter(false));
setTimeout(()=>syncCenter(false),0);
